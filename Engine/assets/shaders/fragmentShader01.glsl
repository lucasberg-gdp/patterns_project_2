...
//		outputColour.rgb = BlurScreenFaster(5);
	...
	if (isBeholder)
	{
		if ( fColour.x > 0.30f && (fOriginalPos.y > 0.0f || (fOriginalPos.x > 1.0f || fOriginalPos.x < -1.0f )))
		{
			vertexRGBA = vec4(239.0 / 255.0, 203.0 / 255.0, 233.0 / 255.0, 255.0 / 255.0);
			outputColour = vertexRGBA;
		}
	}
}
